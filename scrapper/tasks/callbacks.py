...
from scrapper.utils.constants import store_in_excel, search_phrase, IMAGE_BASE_PATH, contains_currency
...
def handle_terms_popup_callback(browser_driver, target, **kwargs):
    logger.info("[INFO]: Closing popup dialogs")
    try:
...
def load_next_page_callback(browser_driver, offset):
    logger.info("[INFO]: Loading next batch of news.")
    try:
...
def extract_news_callback(parent_element, offset):
    logger.info("[INFO]: Extracting news element from page")
    http_client = HTTP()
    news_count = 1
    ...
    for news_element in news_elements:
        ...
        try:
            ...
            logger.info("[INFO]: Skip advertisement element")
            news_count += 1
            continue
        except NoSuchElementException:
        ...
        # save file
        if news_image:
            logger.info("[INFO]: Downloading news feature image")
            ...
            logger.info("[INFO]: News feature image download complete")
