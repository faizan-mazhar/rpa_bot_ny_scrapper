...
def generate_date_range(num_of_months=0):
    ...
    if num_of_months == 0 or num_of_months == 1:
        ...
        return (datetime.strptime(f"01{end_month}{end_year}", DATE_FORMAT).strftime(DATE_STR_FORMAT),
                datetime.strptime(f"{end_day}{end_month}{end_year}", DATE_FORMAT).strftime(DATE_STR_FORMAT)
                )
    ...
    return (
        datetime.strptime(f"01{start_month}{start_year}", DATE_FORMAT).strftime(DATE_STR_FORMAT),
        datetime.strptime(f"{end_day}{end_month}{end_year}", DATE_FORMAT).strftime(DATE_STR_FORMAT)
    )
